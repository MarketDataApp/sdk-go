...
Fetch one daily candle per symbol. Unlike [Candles](./candles.md), this endpoint returns a single candle for each symbol, not a time series.
...
Represents a single daily candle for one symbol. It carries the same OHLCV fields as [`Candle`](./candles.md) plus the `Symbol` the candle belongs to, since bulk requests cover multiple symbols. Timestamps are normalized to US Eastern time.
